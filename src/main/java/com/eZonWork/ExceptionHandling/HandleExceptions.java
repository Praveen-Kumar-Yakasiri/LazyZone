...
public class HandleExceptions  {
	    ...
	    public ResponseEntity<CommonResponse<Object>> handleExpiredTokenException(ExpiredJwtException ex) {
	        ...
	        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(CommonResponse.of(CommonConstant.ErrorCode, CommonConstant.Error, ex.getMessage(), null));
	    }
	    ...
	    public ResponseEntity<CommonResponse<Object>> handleAccesDenied(AccessDeniedException ex) {
	    	System.err.println("handled exception");
	        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(CommonResponse.of(String.valueOf(HttpStatus.FORBIDDEN.value()), HttpStatus.FORBIDDEN.toString().substring(4),null, ex.getMessage(), null));
	    }
